    ...
    let variant_structs = variants.iter().map(|variant| {
        ...
        let impl_fields = variant.accounts.iter().map(|account| {
            ...
            if account.optional {
                ...
                quote! {
                    let #account_name = domichain_program::account_info::next_account_info(account_info_iter)?;
                }
            }
        });
        ...
        quote! {
            ...
            impl<'a> #name<'a> {
                pub fn to_context(accounts: &'a [domichain_program::account_info::AccountInfo<'a>]) -> Result<Context<'a, Self>, domichain_program::sysvar::slot_history::ProgramError> {
                    let account_info_iter = &mut accounts.iter();
