    ...
    fn add_reservation(
        ...
        while self.reservations.len() < usize_offset {
            self.reservations.push(Reservation {
                address: domichain_program::system_program::ID,
                spots_remaining: 0,
                total_spots: 0,
            })
        }
        if self.reservations.len() > usize_offset {
            ...
            } else if replaced_addr != domichain_program::system_program::ID {
        ...
        if usize_offset != 0
            && self.reservations[usize_offset - 1].address == domichain_program::system_program::ID
        {
